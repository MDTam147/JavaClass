public class RegexPatterns {
	...
	public static boolean testName(String name){
		return name.matches("[A-Z]{1,2}[a-z]*(-[A-Z][a-z]*){0,1}\s[A-Z]{1,2}[a-z]*(-[A-Z][a-z]*){0,1}");
	}
	...
	public static boolean testEmail(String email){
		return email.matches("([A-Z] | [a-z])(\\w | \\- | \\.)*\\@([A-Z] | [a-z])(\\w | \\- )*\\.[com|org|net]");
	}
	...
	public static boolean testAddress(String address){
		return address.matches("[0-9]{1,5}\s([A-Z] | [a-z] | [0-9] | \s | \\. | \\-)+\n([A-Z] | [a-z] | \s | \\. | \\-)+\s?,?\s?[A-Z}{2}\s+[0-9]{5}");
	}
